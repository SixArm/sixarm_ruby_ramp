...
class YAMLTest < Test::Unit::TestCase
 ...
 def test_load_dir_key_values
  ...
  actual=[]
   YAML.load_dir_key_values(dirpath){|key,values| actual << "#{key}:#{values.sort.join(",")}" }.sort.join(";") 
   actual.sort!
   assert_equal(expect,actual,'YAML.load_dir_key_values')
 end
